...
from proximal import block_thresh, block_thresh_2d, block_thresh_entrywise, prox_nuc
from util import pack_and_stack, partial_coher
...
def AIC(SX_, S_, L_, ei_thresh=1e-6):
    ...
    F, p, _ = S_.shape
    R_ = S_ - L_
    rank = sum([np.linalg.matrix_rank(L_[f]) for f in xrange(L_.shape[0])])
...
def BIC_split(SX_, S_, L_, n_samples, ei_thresh=1e-6):
    ...
    ranks = [np.linalg.matrix_rank(L_[f]) for f in xrange(L_.shape[0])]
    rank = np.sum(ranks)
